...
run = S.Builtin "run" $ \case
  (S.EPath p) -> return $ S.EBuiltin $ S.Builtin "run" $ \case
    ...
    y -> error $ printf "Expected a [String] as the second argument to `run', got %s." (show y)
  x -> error $ printf "Expected a Path as the first argument to `run', got %s." (show x)
...
cd = S.Builtin "cd" $ \case
  S.EPath p ->
    if not $ p ^. S.pathIsDirectory
    then error $ printf "Expected a directory as the argument to `cd', got %s." (show p)
    else do
      p' <- S.pathToFilePath p
      D.setCurrentDirectory p'
      return $ S.EUnit S.TushUnit
  x -> error $ printf "Expected a directory Path as the argument to `cd', got %s." (show x)
...
moveTo = S.Builtin "moveTo" $ \case
  S.EPath p -> return $ S.EBuiltin $ S.Builtin "moveTo" $ \case
    S.EPath q ->
      ...
      then error $ printf "Expected a directory Path as the second argument to `moveTo', got %s." (show q)
      else do
        p' <- S.pathToFilePath p
        q' <- S.pathToFilePath $
        ...
        D.renamePath p' q'
        return $ S.EUnit S.TushUnit
    y -> error $ printf "Expected a directory Path as the second argument to `moveTo', got %s." (show y)
  x -> error $ printf "Expected a Path as the first argument to `moveTo', got %s." (show x)
