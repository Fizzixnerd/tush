...
getElementPtr :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> [Operand] -> m Operand
...
gepReturnType (PointerType t as) ((ConstantOperand (C.Int 64 _)):is) =
  gepReturnType t is

ptrToInt :: (MonadState LlvmState m, MonadThrow m) => Operand -> m Operand
ptrToInt p = instr Nothing $ A.PtrToInt p int []

bitcast :: (MonadState LlvmState m, MonadThrow m) => Operand -> A.Type -> m Operand
bitcast x t = instr Nothing $ A.BitCast x t []

fadd :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
fadd a b = instr Nothing $ A.FAdd NoFastMathFlags a b []

add :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
add a b = instr Nothing $ A.Add False False a b []

fsub :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
fsub a b = instr Nothing $ A.FSub NoFastMathFlags a b []

sub :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
sub a b = instr Nothing $ A.Sub False False a b []

fmul :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
fmul a b = instr Nothing $ A.FMul NoFastMathFlags a b []

mul :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
mul a b = instr Nothing $ A.Mul False False a b []

fdiv :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
fdiv a b = instr Nothing $ A.FDiv NoFastMathFlags a b []

div :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
div a b = instr Nothing $ A.SDiv False a b []

fcmp :: ( MonadState LlvmState m, MonadThrow m ) => FPP.FloatingPointPredicate -> Operand -> Operand -> m Operand
fcmp cond a b = instr Nothing $ A.FCmp cond a b [] 

icmp :: ( MonadState LlvmState m, MonadThrow m ) => IP.IntegerPredicate -> Operand -> Operand -> m Operand
icmp cond a b = instr Nothing $ A.ICmp cond a b []

flt :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
flt a b = fcmp FPP.ULT a b

lt :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
lt a b = icmp IP.SLT a b

--- Control Operations (Terminators)

term :: (MonadState LlvmState m, MonadThrow m) => 
...
br :: ( MonadState LlvmState m, MonadThrow m ) => Name -> m (Named Terminator)
br val = term $ Do $ Br val []

cbr :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Name -> Name -> m (Named Terminator)
cbr cond tr fl = term $ Do $ CondBr cond tr fl []

ret :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> m (Named Terminator)
ret val = term $ Do $ Ret (Just val) []

toArg :: Operand -> (Operand, [PA.ParameterAttribute])
toArg arg = (arg, [])

call :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> [Operand] -> m Operand
call fn args = instr Nothing $ Call Nothing CC.C [] (Right fn) (toArg <$> args) [] []

--- Memory Operations

alloca :: ( MonadState LlvmState m, MonadThrow m ) => Type -> m Operand
alloca t = instr Nothing $ Alloca t Nothing 0 []

store :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> Operand -> m Operand
store ptr val = instr Nothing $ Store False ptr val Nothing 0 []

load :: ( MonadState LlvmState m, MonadThrow m ) => Operand -> m Operand
load ptr = instr Nothing $ Load False ptr Nothing 0 []

loadInto :: (MonadState LlvmState m, MonadThrow m) => 
